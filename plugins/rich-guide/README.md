...
flowchart TD
    ...
    C -->|없음 / 30일 초과| F["7-Field 재무 인터뷰<br/>월수입/월지출/예금/투자자산<br/>부채/리스크성향/목표"]

    D & E & F --> G[("profiles.db 저장<br/>~/.claude/skills/rich-guide/data/")]

    G --> H[Phase 1: 병렬 분석]

    subgraph Phase1["Phase 1: 병렬 분석"]
        direction LR
        P1A["financial-diagnostician<br/>(Sonnet)<br/>재무 건강도 0-100점 산출"]
        P1B["info-curator<br/>(Haiku)<br/>화이트리스트 기반<br/>재테크 정보 수집"]
        P1C["market-context-analyzer<br/>(Sonnet)<br/>현재 금리/시장 상황 분석"]
    end
    ...
    subgraph Phase2["Phase 2: 순차 전략 생성 + 평가"]
        J2A["wealth-strategist<br/>(Opus)<br/>3-5개 개인화 전략 생성<br/>리스크x시간x분야 조합"]
        J2B["risk-reward-evaluator<br/>(Sonnet)<br/>전략별 리스크/보상 정량 평가"]
        J2A --> J2B
    end

    J --> Phase2
    J2B --> K["전략 선택<br/>AskUserQuestion"]
    K --> L[Phase 3: 실행 계획 생성]

    subgraph Phase3["Phase 3: 실행 계획"]
        L3["action-plan-generator<br/>(Opus)<br/>roadmap-template.md 읽기<br/>12주 주간 체크리스트 생성"]
    end
